...
#define SD_SPI_NUMBER SPI_DEVICE_1
/*
...
static uint8_t sd_cs_func_gpiohs_num = 0;

void SD_CS_HIGH(void)
{
    gpiohs_set_pin(sd_cs_func_gpiohs_num, GPIO_PV_HIGH);
}

void SD_CS_LOW(void)
{
    gpiohs_set_pin(sd_cs_func_gpiohs_num, GPIO_PV_LOW);
}

void SD_HIGH_SPEED_ENABLE(void)
{
    spi_set_clk_rate(SD_SPI_NUMBER, 10000000);
}

static void sd_lowlevel_init(uint8_t spi_index)
{
    gpiohs_set_drive_mode(sd_cs_func_gpiohs_num, GPIO_DM_OUTPUT);
    spi_set_clk_rate(SD_SPI_NUMBER, 200000);     /*set clk rate*/
}

static void sd_write_data(uint8_t *data_buff, uint32_t length)
{
    spi_init(SD_SPI_NUMBER, SPI_WORK_MODE_0, SPI_FF_STANDARD, 8, 0);
    spi_send_data_standard(SD_SPI_NUMBER, SPI_CHIP_SELECT_3, NULL, 0, data_buff, length);
}
...
static void sd_read_data(uint8_t *data_buff, uint32_t length)
{
    ...
    spi_init(SD_SPI_NUMBER, SPI_WORK_MODE_0, SPI_FF_STANDARD, 8, 0);
    spi_receive_data_standard(SD_SPI_NUMBER, SPI_CHIP_SELECT_3, NULL, 0, data_buff, length);

}

static void sd_write_data_dma(uint8_t *data_buff)
{
    spi_init(SD_SPI_NUMBER, SPI_WORK_MODE_0, SPI_FF_STANDARD, 32, 1);
    spi_send_data_standard_dma(DMAC_CHANNEL0, SD_SPI_NUMBER, SPI_CHIP_SELECT_3, NULL, 0, (uint8_t *)(data_buff), 128 * 4);
}

static void sd_read_data_dma(uint8_t *data_buff)
{
    spi_init(SD_SPI_NUMBER, SPI_WORK_MODE_0, SPI_FF_STANDARD, 32, 1);
    spi_receive_data_standard_dma(-1, DMAC_CHANNEL0, SD_SPI_NUMBER, SPI_CHIP_SELECT_3,NULL, 0, data_buff,128 * 4);
}
...
/*
 * @brief  set sd chip select num for gpiohs.
 * @param  cs_func_gpiohs_num
 * @retval set OK or not:
 *         - 0xFF: set ERROR
 *         - 0: set OK
 */
uint8_t set_sd_cs_num(uint8_t cs_func_gpiohs_num)
{
	if(cs_func_gpiohs_num < 0 || cs_func_gpiohs_num > 32){
		printf("cs_func_gpiohs_num error(must >= 0 && <= 32)\n");
		return 0xFF;
	}
	sd_cs_func_gpiohs_num = cs_func_gpiohs_num;
	return 0;
}
